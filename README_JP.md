...
IRIS は、W3C WCAG および ISO 9241-391 に記載された検出方法の実装を目指し、光過敏性てんかんを持つ人々に発作を引き起こす可能性のあるフラッシュパターンを動画コンテンツから検出・解析します。
...
本ソフトウェアは情報提供のみを目的としており、動画コンテンツの安全性を確保する唯一の方法として使用すべきではありません。本実装はオリジナルの IRIS ライブラリに基づいていますが、精度や規格への準拠について保証するものではありません。本番環境での使用には、アクセシビリティの専門家への相談と適切なテストを必ず実施してください。
